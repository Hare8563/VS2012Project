...
namespace LabImg
{
	...
	public static class Collision
	{
        ...
        public static bool JudgeTrianglePlane(PVector p1, PVector p2, Triangle t, out PVector v)
        {
            bool collision = false;
            ...
            if (Sn * En < 0)
            {
                float d1 = (float)(Math.Abs(Sn) / Math.Sqrt(Math.Pow(normal.X, 2) + Math.Pow(normal.Y, 2) + Math.Pow(normal.Z, 2)));
                float d2 = (float)(Math.Abs(En) / Math.Sqrt(Math.Pow(normal.X, 2) + Math.Pow(normal.Y, 2) + Math.Pow(normal.Z, 2)));

                float a = d1 / (d1 + d2);

                //PVector i = new PVector((p1.X * En + p2.X * Sn) / (Sn + En), (p1.Y * En + p2.Y * Sn) / (Sn + En), (p1.Z * En + p2.Z * Sn) / (Sn + En));
                PVector vv = new PVector((1 - a) * start.X + a * end.X, (1 - a) * start.Y + a * end.Y, (1 - a) * start.Z + a * end.Z);
                PVector i = new PVector(p.X + vv.X, p.Y + vv.Y, p.Z + vv.Z);
                
                v = i;
                PVector v1 = new PVector(i.X - t.V1.X, i.Y - t.V1.Y, i.Z - t.V1.Z);
                PVector v2 = new PVector(i.X - t.V2.X, i.Y - t.V2.Y, i.Z - t.V2.Z);
                PVector v3 = new PVector(i.X - t.V3.X, i.Y - t.V3.Y, i.Z - t.V3.Z);

                PVector AB = new PVector(t.V2.X - t.V1.X, t.V2.Y - t.V1.Y, t.V2.Z - t.V1.Z);
                PVector BC = new PVector(t.V3.X - t.V2.X, t.V3.Y - t.V2.Y, t.V3.Z - t.V2.Z);
                PVector CA = new PVector(t.V1.X - t.V3.X, t.V1.Y - t.V3.Y, t.V1.Z - t.V3.Z);

                PVector n1 = v1.Cross(AB);
                PVector n2 = v2.Cross(BC);
                PVector n3 = v3.Cross(CA);

                float n1n = n1.Dot(normal);
                float n2n = n2.Dot(normal);
                float n3n = n3.Dot(normal);

                if (n1n * n2n >= 0 && n2n * n3n >= 0 && n1n * n3n >= 0)
                {
                    collision = true;
                }
                else
